 ...
 * i18n: react-i18next, zh/en, localStorage persisted
 *
 * 结构：
 *   ① 顶部品牌标识 + 语言切换（中/EN）
 *   ② AI 气象核心展示（完整天气页内容）
...
import {
  ChevronLeft, ChevronRight, ZoomIn, X, Mail,
  Wind, Zap, Leaf, Building2, Code2, BookOpen,
} from "lucide-react";
...
// Capability card icons (order matches i18n capabilities.items array)
const CAP_ICONS = [Wind, Zap, Leaf, Building2, Code2, BookOpen];

// Tags per language (not in i18n JSON to keep it simple)
const CAP_TAGS_ZH = [
  ["FuXi / Pangu-Weather", "NVIDIA Earth-2", "GFS 初始场", "7天预报"],
  ["双碳分析", "风光资源评估", "碳排放核算", "能源结构"],
  ["AQI 实时监测", "污染物溯源", "环境数据可视化", "多站点聚合"],
  ["WebGIS", "空间数据分析", "城市热岛", "三维可视化"],
  ["AI Coding", "全栈部署", "LLM 应用", "Cloud / Docker"],
  ["AI气象", "新能源", "工程实践", "持续更新"],
];
const CAP_TAGS_EN = [
  ["FuXi / Pangu-Weather", "NVIDIA Earth-2", "GFS Initial Field", "7-Day Forecast"],
  ["Dual-Carbon Analysis", "Wind/Solar Assessment", "Carbon Accounting", "Energy Planning"],
  ["Real-time AQI", "Pollution Tracing", "Env Data Viz", "Multi-station"],
  ["WebGIS", "Spatial Analysis", "Urban Heat Island", "3D Visualization"],
  ["AI Coding", "Full-stack Deploy", "LLM Apps", "Cloud / Docker"],
  ["AI Weather", "Renewable Energy", "Engineering", "Ongoing"],
];
...
function LangToggle() {
  ...
  const toggle = () => {
    ...
    localStorage.setItem("wsl-lang", next);
  };
  return (
    <button
      onClick={toggle}
      className="flex items-center gap-1 px-3 py-1.5 rounded border border-border text-xs font-mono text-muted-foreground hover:text-primary hover:border-primary/50 transition-colors"
      title={isEn ? "切换到中文" : "Switch to English"}
    >
      <span className={isEn ? "text-muted-foreground/40" : "text-primary font-bold"}>中</span>
      <span className="text-border mx-0.5">/</span>
      <span className={isEn ? "text-primary font-bold" : "text-muted-foreground/40"}>EN</span>
    </button>
  );
}
...
// ─── FigureCard ───────────────────────────────────────────────────────────────
function FigureCard({
  fig, figIdx, dayIdx, onZoom,
}: {
  fig: WeatherFigure; figIdx: number; dayIdx: number;
  onZoom: (fig: WeatherFigure, figIdx: number, dayIdx: number) => void;
}) {
  ...
  const titles = t("figTitles", { returnObjects: true }) as string[];
  const explanations = t("figExplanations", { returnObjects: true }) as string[];
  const title = titles[figIdx] ?? fig.title;
  const explanation = explanations[figIdx] ?? fig.explanation;
  ...
  return (
    <div className="wsl-card overflow-hidden group flex flex-col">
      <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-2 flex-shrink-0">
        ...
        <button
          onClick={() => onZoom(fig, figIdx, dayIdx)}
          ...
          title={t("weather.zoomIn")} aria-label={t("weather.zoomIn")}
        >
          <ZoomIn size={13} />
        </button>
      </div>
      ...
      >
        {containerW > 0 && (
          <img
            src={fig.src}
            alt={`${title} — ${FUXI_DATES[dayIdx].label}`}
            style={{
              height: "100%", width: "auto", maxWidth: "none", display: "block",
              marginLeft: `${marginLeft}px`, transition: "margin-left 0.25s ease-in-out",
            }}
          />
        )}
...
// ─── Lightbox ─────────────────────────────────────────────────────────────────
function Lightbox({
  fig, figIdx, dayIdx, onClose, onPrevDay, onNextDay, onSetDay,
}: {
  fig: WeatherFigure; figIdx: number; dayIdx: number;
  onClose: () => void; onPrevDay: () => void; onNextDay: () => void; onSetDay: (i: number) => void;
}) {
  const { t } = useTranslation();
        ...
        >
          {imgH > 0 && (
            <img
              ...
              style={{
                height: "100%", width: "auto", maxWidth: "none", display: "block",
                marginLeft: `${marginLeft}px`, transition: "margin-left 0.3s ease-in-out",
              }}
            />
          )}
          <button
...
export default function ShareWeather() {
  ...
  const [lightbox, setLightbox] = useState<{ fig: WeatherFigure; figIdx: number; dayIdx: number } | null>(null);

  const openLightbox = (fig: WeatherFigure, figIdx: number, d: number) =>
    setLightbox({ fig, figIdx, dayIdx: d });
  ...
  const isEn = i18n.language === "en";
  const capTags = isEn ? CAP_TAGS_EN : CAP_TAGS_ZH;
  const capItems = t("capabilities.items", { returnObjects: true }) as Array<{
    title: string; subtitle: string; desc?: string;
  }>;
  const noteTitles = t("notes.titles", { returnObjects: true }) as string[];

  return (
    <div className="min-h-screen bg-background text-foreground">

      {/* ① Brand header — no navigation, language toggle only */}
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-2 max-w-xl leading-relaxed">
                {t("weather.desc", { initTime: WEATHER_META.initTime })}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {(t("weather.tags", { returnObjects: true }) as string[]).map((tag) => (
                <span key={tag}
      ...
      <div className="border-b border-border bg-muted/5">
        <div className="container py-4">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs text-muted-foreground font-mono mr-1">{t("weather.selectDate")}</span>
            <button
            ...
            <span className="text-xs text-muted-foreground font-mono hidden sm:inline">
              {isEn ? "· 6 charts sync · click to zoom" : "· 6张图同步联动 · 点击图片可放大"}
            </span>
          </div>
        </div>
      </div>
      ...
      <section className="py-10">
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FUXI_FIGURES.map((fig, idx) => (
              <FigureCard key={fig.id} fig={fig} figIdx={idx} dayIdx={dayIdx} onZoom={openLightbox} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-muted/5 border-t border-border">
        <div className="container">
          ...
          <div className="flex flex-wrap items-center gap-2 justify-center mb-10">
            {[
              { step: "01", label: "GFS 0.25°", sub: "NOAA" },
              null,
              { step: "02", label: "FuXi", sub: isEn ? "~34 min" : "约34分钟" },
              null,
              { step: "03", label: "zarr", sub: isEn ? "6-var storage" : "6变量存储" },
              null,
              { step: "04", label: isEn ? "Visualization" : "6变量可视化", sub: "Python matplotlib" },
            ].map((item, i) =>
              item === null ? (
          ...
          <div className="max-w-2xl mx-auto">
            <h3 className="text-base font-semibold text-foreground mb-4"
              style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              {isEn ? "Technical Parameters" : "技术参数"}
            </h3>
            <div className="wsl-card overflow-hidden">
              <table className="w-full text-sm">
                <tbody>
                  {(isEn
                    ? [
                        ["Model", WEATHER_META.model],
                        ["Framework", WEATHER_META.framework],
                        ["Initial Data", WEATHER_META.initData],
                        ["Resolution", WEATHER_META.resolution],
                        ["Forecast Range", WEATHER_META.forecastHours],
                        ["Region", WEATHER_META.region],
                        ["Inference Time", WEATHER_META.inferenceTime],
                        ["Output Variables", WEATHER_META.outputVars],
                        ["Init Time", WEATHER_META.initTime],
                        ["Pipeline", WEATHER_META.pipeline],
                      ]
                    : [
                    ...
                    <tr key={key} className={i % 2 === 0 ? "bg-muted/10" : ""}>
                      <td className="px-4 py-2.5 font-mono text-xs text-primary/80 whitespace-nowrap w-32 align-top">
                        {key}
                      </td>
      ...
      <section className="py-16 border-t border-border">
        <div className="container">
          ...
          <p className="text-center text-sm text-muted-foreground mb-10 font-mono">
            {t("capabilities.sectionDesc")}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {capItems.map((cap, i) => {
              const Icon = CAP_ICONS[i] ?? Wind;
              const isNotes = i === 5;
              return (
                <div key={i} className="wsl-card p-5 flex flex-col gap-3 hover:border-primary/40 transition-colors">
                  ...
                  {!isNotes ? (
                    ...
                    <ul className="space-y-2">
                      {noteTitles.slice(0, 2).map((noteTitle, ni) => (
                        <li key={ni} className="flex items-start gap-2">
                          <span className="mt-1.5 w-1 h-1 rounded-full bg-primary/50 flex-shrink-0" />
                          <div className="min-w-0">
                            <p className="text-xs text-foreground/80 leading-snug line-clamp-2">{noteTitle}</p>
                      ...
                      <li className="text-[10px] font-mono text-muted-foreground/40 pl-3">
                        {t("capabilities.notesMore")}
                      </li>
                    </ul>
                  )}
        ...
        <div className="container py-8">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{t("footer.labDesc1")}</p>
              <p className="text-xs text-muted-foreground mt-1">{t("footer.labDesc2")}</p>
            </div>
            <div className="flex flex-col gap-2">
              ...
              <a href="mailto:wenzhuopan26@gmail.com"
            ...
            <div className="text-xs text-muted-foreground space-y-1 text-right">
              <p>{t("footer.dataAttr1")}</p>
              <p>{t("footer.dataAttr2")}</p>
              <p>{t("footer.dataAttr3")}</p>
            </div>
          </div>
          <div className="mt-6 pt-4 border-t border-border/50 flex items-center justify-between">
            <p className="text-xs text-muted-foreground/60">{t("footer.copyright")}</p>
            <p className="text-xs font-mono text-muted-foreground/40">v0.1.0-mvp</p>
          </div>
        </div>
      </footer>
